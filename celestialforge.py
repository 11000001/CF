...
settings.configure()
django.setup()
from celestial_forge_site.celestial_forge.models import Domain as mDomain
from celestial_forge_site.celestial_forge.models import Perk as mPerk
from celestial_forge_site.celestial_forge.models import Addon as mAddon
...
def populate_django_models ():
	...
	for domain in cf.domains:
		...
		for perk in domain.perks:
			p = mPerk(name=perk.name, description=perk.description, cost=perk.cost, source=perk.source, domain=d)
			try:
				p.save()
			except:
				print("Problem", perk)
